...
for i in range(start_from - 1, n_runs):
    ...
    run_results_file = results_file.with_name(
        results_file.stem + f"_run_{i+1}" + results_file.suffix
    )
    ...
    script.add_to_block(
        ...
        incawrapper.define_runner(
            output_filename=run_results_file,
            run_estimate=True,
    ...
    time_spend = np.append(time_spend, end_time - start_time)  # Store the time spent

    # print predicted time to finish
    avg_time_spend = np.mean(time_spend)
    print(f"Average time spent: {avg_time_spend}")
    print(f"Predicted time to finish: {avg_time_spend*(n_runs-i-1)}")
