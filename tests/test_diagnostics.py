...
def test_plot_residuals_vs_fitted(inca_results):
    """
    Tests if the plot_residuals_vs_fitted function runs without errors
    and returns an altair chart
    """
    assert type(plot_residuals_vs_fitted(inca_results)) == alt.Chart

def test_plot_norm_probplot_standard_plot(inca_results):
    """
    Tests if the plot_norm_probplot function runs without errors and 
    returns a matplotlib figure
    """
    assert type(plot_norm_probplot(inca_results)[0]) == plt.Figure

def test_plot_norm_probplot_interactive_plot(inca_results):
    """
    Tests if the plot_norm_probplot with interactive option function
    runs without errors and returns an altair chart
    """

    assert type(plot_norm_probplot(inca_results, interactive=True)) == alt.LayerChart